...
## Final checked package

В финальной проверенной сборке исправлены:

- GitHub Actions: сборка идёт через Node.js 22.12.0 и `npm ci`, без pnpm/corepack.
- Виджет «Версия для слабовидящих»: теперь это компактная кнопка 👁, панель открывается поверх страницы и не сдвигает контент.
- Админка: при создании новой записи `id` не отправляется как `null`, Supabase генерирует его сам.
- SQL-миграции: добавлен `supabase/final-repair.sql`, который безопасно чинит `metadata`, default для `id`, права и RLS.

Если в админке появляются ошибки `metadata column` или `null value in column id`, выполни в Supabase SQL Editor:
